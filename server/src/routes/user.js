...
router.post('/register', async (req, res) => {
    try {
        ...
        // 更新用户名（如果提供）
        if (username) {
            await getDatabase().runAsync('UPDATE users SET username = ?, updated_at = datetime(\'now\') WHERE id = ?', [username, user.id]);
        }
        // 生成访问令牌
...
router.post('/orders/:orderId/pay', authMiddleware, async (req, res) => {
    try {
        ...
        await initDatabase();
        // 获取订单信息
        const order = await getDatabase().getFirstAsync('SELECT * FROM orders WHERE id = ? AND user_id = ?', [parseInt(orderId), userId]);
        if (!order) {
