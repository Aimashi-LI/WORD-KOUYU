...
import { createWordbook, addWordsToWordbook } from './wordbookDao';
 ...
 * 3. 将单词与词库关联
 */
export async function importHighFrequencyWords(): Promise<number> {
  try {
    console.log('[importHighFrequencyWords] 开始导入高频100词...');
    ...
    if (existingBook) {
      console.log('[importHighFrequencyWords] 词库"高频100词"已存在，跳过创建');
      wordbookId = existingBook.id;
    } else {
      // 创建"高频100词"词库
      console.log('[importHighFrequencyWords] 创建"高频100词"词库...');
      const result = await createWordbook('高频100词', '高中高频英语单词前100个');
      wordbookId = result;
      console.log('[importHighFrequencyWords] 词库创建成功，ID:', wordbookId);
    }
    
    // 批量插入单词
    console.log('[importHighFrequencyWords] 插入100个单词...');
    ...
    for (const wordData of HIGH_FREQUENCY_WORDS) {
      ...
      if (existingWord) {
        ...
        console.log(`[importHighFrequencyWords] 插入单词 "${wordData.word}"，ID: ${wordId}`);
      }
    ...
    // 将单词与词库关联
    console.log(`[importHighFrequencyWords] 将${wordIds.length}个单词与词库关联...`);
    await addWordsToWordbook(wordbookId, wordIds);
    
    console.log('[importHighFrequencyWords] ✅ 高频100词导入完成！');
    console.log(`[importHighFrequencyWords] 词库ID: ${wordbookId}`);
    console.log(`[importHighFrequencyWords] 单词数量: ${wordIds.length}`);
    
    return wordbookId;
  } catch (error) {
    console.error('[importHighFrequencyWords] 导入失败:', error);
    throw error;
  }
}
